...
SELECT * FROM 
	...
	).std:groupwin(ip_src,ip_dst,service,password).win:time_length_batch(${time_window?c} seconds, ${count?c}).std:unique(username) group by ip_src, ip_dst, service having count(*) = ${count?c};
