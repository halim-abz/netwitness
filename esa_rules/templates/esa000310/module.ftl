...
@RSAAlert(oneInSeconds=${alert_suppression?c}, identifiers={"ip_src","ip_dst","service","username"})
...
SELECT * FROM 
	...
	).std:unique(ip_src,ip_dst,service,username) group by ip_src,ip_dst,service,username<#if alert_suppression != 0> output first every ${alert_suppression/60} min</#if>;
