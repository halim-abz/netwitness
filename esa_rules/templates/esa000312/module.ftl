...
@RSAAlert(oneInSeconds=${alert_suppression?c}, identifiers={"ip_src","filename"})
...
SELECT * FROM 
	...
	).std:unique(ip_src,filename) group by ip_src,filename<#if alert_suppression != 0> output first every ${alert_suppression/60} min</#if>;
