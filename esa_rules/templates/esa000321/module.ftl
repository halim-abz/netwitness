...
SELECT * FROM 
	Event(
	    medium = 1
	    AND 'echo' = ANY(action)
	    AND 'first carve' = ANY(analysis_session)
		AND entropy_req > ${entropy_size?c}
		<#if org_list[0].value != "">
		AND org_dst.toLowerCase() NOT IN (<@buildList inputlist=org_list/>)
		AND org_src.toLowerCase() NOT IN (<@buildList inputlist=org_list/>)
		</#if>
	...
	).std:unique(ip_src,ip_dst) group by ip_src,ip_dst<#if alert_suppression != 0> output first every ${alert_suppression/60} min</#if>;
