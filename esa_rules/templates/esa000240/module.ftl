...
SELECT * FROM 
	...
	).std:groupwin(ip_src, tcp_dstport).win:time_length_batch(${time_window?c} seconds, ${count?c}).std:unique(ip_dst) group by ip_src, tcp_dstport having count(*) = ${count?c};
