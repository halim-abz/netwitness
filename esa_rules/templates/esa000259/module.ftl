...
SELECT * FROM 
	Event(
		medium = 1
		AND isOneOfIgnoreCase(alias_host,{ 'api.telegram.org' })
		AND
		(
			( service = 443 AND isOneOfIgnoreCase(analysis_session,{ 'ratio high transmitted' }) AND payload > ( 10485760 ) )
			OR
			(service = 80 AND (filename IS NOT NULL OR attachment IS NOT NULL) )
		)
		<#if ip_list[0].value != "">
