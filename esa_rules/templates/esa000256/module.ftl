...
SELECT window(*) FROM 
	...
	).std:groupwin(ip_src).win:time_length_batch(${time_window?c} seconds, ${count*2}).std:unique(client) group by ip_src having count(*) >= ${count?c} output first every 30 min;
