...
SELECT window(*) FROM 
	...
	).std:groupwin(ip_src,ip_dst,username).win:time_length_batch(${time_window?c} seconds, ${count*2}) group by ip_src,ip_dst,username having count(*) >= ${count?c} output first every 30 min;
