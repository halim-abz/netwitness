...
SELECT * FROM 
	...
	).std:groupwin(ip_src,domain).win:time_length_batch(${time_window?c} seconds, ${count?c}).std:unique(alias_host) group by ip_src,domain,service having count(*) = ${count?c};
