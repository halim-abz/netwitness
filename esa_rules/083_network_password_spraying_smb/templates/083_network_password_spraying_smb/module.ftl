...
SELECT * FROM 
	...
	).std:groupwin(ip_src,ip_dst,password).win:time_length_batch(${time_window?c} seconds, ${count?c}).std:unique(ad_username_src) group by ip_src, ip_dst having count(*) = ${count?c};
