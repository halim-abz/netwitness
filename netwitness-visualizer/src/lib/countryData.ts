...
export function getIsoA2FromGeoJson(properties: any): string | null {
  if (!properties) return null;
  
  // Direct ISO_A2 match (excluding -99 which Natural Earth uses for missing/special cases)
  if (properties.ISO_A2 && properties.ISO_A2 !== '-99') {
    return properties.ISO_A2;
  }
  
  // Fallback map for common -99 cases or missing ISO_A2
  ...
  if (properties.NAME && nameMap[properties.NAME]) {
    return nameMap[properties.NAME];
  }
  
  // Try to find by name in COUNTRY_DATA
  if (properties.NAME) {
    const nameLower = properties.NAME.toLowerCase();
    for (const [code, data] of Object.entries(COUNTRY_DATA)) {
      if (data.name.toLowerCase() === nameLower) {
        return code;
      }
    }
  }
